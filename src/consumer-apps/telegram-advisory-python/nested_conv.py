...
(FIRST_NAME, FATHERS_NAME, KEBELE, CORRECT_FIRST_NAME, CORRECT_FATHERS_NAME, 
 CORRECT_KEBELE) = map(chr, range(10, 16))
...
def _name_switcher(level):
    if level == CORRECT_FIRST_NAME:
        return ('First name')
    elif level == CORRECT_FATHERS_NAME:
        return ('Fathers name')
    elif level == CORRECT_KEBELE:
        return ('Kebele')
    elif level == FATHERS_NAME:
        return ('Fathers name')
...
def start(update, context):
    """Select an action: Ask person details or show data."""
    text = 'Hello "Sujit", your number is mentioned in the Woreda office, "DG REG" register.' \
    ...
    buttons = [[
        InlineKeyboardButton(text='Yes', callback_data=str(ADDING_MEMBER)),
    ], [
    ...
    if context.user_data.get(START_OVER):
        ...
        update.message.reply_text('Wheat Rust Bot')
        update.message.reply_text(text=text, reply_markup=keyboard)
...
def select_level(update, context):
    """Choose Sub Level i.e check for a name"""
    text = 'Is your first name Sujit?'
    buttons = [[
        InlineKeyboardButton(text='Yes', callback_data=str(FATHERS_NAME)),
        InlineKeyboardButton(text='No', callback_data=str(CORRECT_FIRST_NAME))
    ]]
    keyboard = InlineKeyboardMarkup(buttons)
...
def main():
    ...
    add_member_conv = ConversationHandler(
        ...
        states={
            SELECTING_LEVEL: [CallbackQueryHandler(select_gender,
                                                   pattern='^{}$|^{}$'.format(str(FIRST_NAME),
                                                                              str(CORRECT_FIRST_NAME)))],
            SELECTING_LEVEL: [CallbackQueryHandler(select_gender,
                                                   pattern='^{}$|^{}$'.format(str(FATHERS_NAME),
                                                                              str(CORRECT_FATHERS_NAME)))],
            SELECTING_LEVEL: [CallbackQueryHandler(select_gender,
                                                   pattern='^{}$|^{}$'.format(str(KEBELE),
                                                                              str(CORRECT_KEBELE)))],
            SELECTING_GENDER: [description_conv]
        },
